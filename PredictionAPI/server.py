...
############### WebService Route /predict/id/<id:int>/pmin/<pmin:float> ###############

@route('/predict/id/<id:int>/pmin/<pmin:float>')
def callbackpmin(id, pmin):
	global classifier
	assert isinstance(id, int)
	...
	if id < 0 :
		strErr = "id parameter can't be negative\n"
	if pmin < 0 :
		strErr = strErr + "pmin parameter can't be negative\n"
	elif pmin > 100 :
		strErr = strErr + "pmin parameter can't be greater than 100\n"
	...
	if strErr != "" :
		...
		if id < maxIndex +1 and id > 0 :
			rslt =  launchPredict(classifier, id, -1, pmin)
			partPredictedClasses = rslt[0]
			partID = rslt[1]
...
######## WebService Route /predict/id/<id:int>/nmax/<nmax:int>/pmin/<pmin:float> ########
@route('/predict/id/<id:int>/nmax/<nmax:int>/pmin/<pmin:float>')
def callback(id, nmax, pmin):
	global classifier
	assert isinstance(id, int)
	assert isinstance(nmax, int)
	assert isinstance(pmin, float)
	...
	if strErr != "" :
		...
		maxIndex = 17
		if id < maxIndex and id > 0 :
			rslt =  launchPredict(classifier, id, nmax, pmin)
			partPredictedClasses = rslt[0]
			partID = rslt[1]
